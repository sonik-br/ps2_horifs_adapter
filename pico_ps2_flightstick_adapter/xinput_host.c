...
static const uint8_t xbox360w_unknown[] = {0x00, 0x00, 0x02, 0x80}; // capabilities
...
bool xinputh_open(uint8_t rhport, uint8_t dev_addr, tusb_desc_interface_t const *desc_itf, uint16_t max_len)
{
    ...
    xid_itf->type = type;
    xid_itf->subtype = 0x01; // use this as default. 0x01 is gamepad on ogXbox and x360
    ...
    {
        // try to get device subtype
        if (type == XBOX360_WIRED && tu_desc_type(p_desc) == 0x21 && tu_desc_len(p_desc) == 0x11)
        {
          xid_itf->subtype = p_desc[4];
        }
...
bool xinputh_set_config(uint8_t dev_addr, uint8_t itf_num)
{
    ...
    else if (xid_itf->type == XBOXOG)
    {
        ...
        if (ret && txbuf[1] == 0x42) {
          uint8_t xid_type = txbuf[4];
          uint8_t xid_subtype = txbuf[5];
          if (xid_type == 0x01) // Xbox Gamecontroller
            xid_itf->subtype = xid_subtype; // (gamepad, gamepad_s, wheel, arcade, lightgun)
          // else if (xid_type == 0x03 && xid_subtype == 0x00) // Xbox DVD Movie Playback IR Dongle
          // else if (xid_type == 0x80 && xid_subtype == 0x01) // Steel Battalion
          else
            xid_itf->subtype = 0x01; // set as gamepad
        }
    }
...
bool xinputh_xfer_cb(uint8_t dev_addr, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
    ...
    if (dir == TUSB_DIR_IN)
    {
        ...
        else if (xid_itf->type == XBOX360_WIRELESS)
        {
            ...
            //Device info packet
            if (rdata[1] == 0x0f && rdata[5] == 0xcc)
            {
              xid_itf->subtype = rdata[25] & 0x0F;

              uint32_t deviceID;// = (rdata[10]) | ((uint32_t)rdata[11] << 8) | ((uint32_t)rdata[12] << 16) | ((uint32_t)rdata[13] << 24);
              memcpy(&deviceID, rdata + 10, 4);
              //printf("Wireless deviceID 0x%08x\n", deviceID);

              //To get a cohesive vendor ID, do ((vendorIDData[0] & 0xf) | vendorIDData[2] << 4) << 8 | vendorIDData[1].
              uint16_t vendorID = ((rdata[22] & 0xf) | rdata[24] << 4) << 8 | rdata[23];
              //printf("Wireless vendorID 0x%04x\n", vendorID);

              if (vendorID == 0x045e && deviceID == 0x30a5c7fc) { // wireless speed wheel
                //need do confirm if deviceID is consistent between multiple devices
                //handy to know as it does not have L1/R1 buttons
                //type 0x0, subtype 0x52, deviceID 0x30A5C7FC, vendorID 0x045E, protocol 0x1D20, revision 0x0
              }

              //todo check how a wireless force feedback wheel reports
        ...
        else if (xid_itf->type == XBOXONE)
        {
            ...
            {
                //Keep a copy of the guide button state
                bool guide_state = (pad->wButtons & XINPUT_GAMEPAD_GUIDE) != 0x0;
                ...
                //Map the left and right triggers
                pad->bLeftTrigger = (rdata[7] << 8 | rdata[6]) >> 2; // todo get it as 10bits?
        ...
        else if (xid_itf->type == XBOXOG)
        {
            if (rdata[1] == 0x14)
            {
                ...
                //Map analog buttons
                pad->sAnalogA = rdata[4];
                pad->sAnalogB = rdata[5];
                pad->sAnalogX = rdata[6];
                pad->sAnalogY = rdata[7];
                pad->sAnalogBlack = rdata[8];
                pad->sAnalogWhite = rdata[9];
